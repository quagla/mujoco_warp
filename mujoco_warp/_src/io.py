...
def make_data(mjm: mujoco.MjModel, nworld: int = 1, nconmax: int = -1, njmax: int = -1) -> types.Data:
  ...
  return types.Data(
    ...
    epa_face=wp.zeros(shape=(nconmax, 6 + 4 * MJ_CCD_ITERATIONS), dtype=wp.vec3i),
    epa_pr=wp.zeros(shape=(nconmax, 6 + 4 * MJ_CCD_ITERATIONS), dtype=wp.vec3),
    epa_norm2=wp.zeros(shape=(nconmax, 6 + 4 * MJ_CCD_ITERATIONS), dtype=float),
    epa_index=wp.zeros(shape=(nconmax, 6 + 4 * MJ_CCD_ITERATIONS), dtype=int),
    epa_map=wp.zeros(shape=(nconmax, 6 + 4 * MJ_CCD_ITERATIONS), dtype=int),
